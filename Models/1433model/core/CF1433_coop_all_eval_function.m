function y = CF1433_coop_all_eval_function(obj)
...
% for different KD values.
    ...
    [EGP, EP, EG,G, E, P] = solve_conservation_equations(obj.parameters,G_name,P_name);
    % storing outputs.
    y = [EGP, EP, EG, G, E, P];
end


function sol = calculate_conservation_equations(E, parameters,G_name,P_name)
% Function calculating the equations that need to be solved for the model.
    ...
    Gt = parameters.Gt;
    % Pt = parameters.Pt;
    Pt = parameters.(append("Pt_",P_name));
    Et = parameters.Et;
...
function  [EGP, EP, EG, G, E, P] = solve_conservation_equations(parameters,G_name,P_name)
% Function for simulating equilibrium model of MG binding kinetics.
    ...
    func = @(y) calculate_conservation_equations(y,parameters,G_name,P_name);
    y0 = [eps Et-eps];
