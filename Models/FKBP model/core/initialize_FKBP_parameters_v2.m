function [parameters, bounds, parameter_names] = initialize_FKBP_parameters_v2()
% Function for setting parameters and bounds for simple equilibrium model
    ...
    % Units: nM
    % Source: estimated
    parameters.KD1_Cmpd5 = 1000;
    bounds.KD1_Cmpd5_bounds = [1e2 1e7];

    % Dissociation constant of E + G <-> EG for Compound 6
    % Units: nM
    % Source: estimated
    parameters.KD1_Cmpd6= 1000;
    bounds.KD1_Cmpd6_bounds = [1e2 1e7];

    % Dissociation constant of E + G <-> EG for Compound 7
    % Units: nM
    % Source: estimated
    parameters.KD1_Cmpd7 = 1000;
    bounds.KD1_Cmpd7_bounds = [1e2 1e7];


    % Dissociation constant of EG + P <-> EGP for Cmpd and FRB
    % Units: nM
    % Source: estimated
    parameters.KD2_Cmpd5_FRB = 1;
    bounds.KD2_Cmpd5_FRB_bounds = [1e-4 1e7];
    parameters.KD2_Cmpd6_FRB = 1;
    bounds.KD2_Cmpd6_FRB_bounds = [1e-4 1e7];
    parameters.KD2_Cmpd7_FRB = 1;
    bounds.KD2_Cmpd7_FRB_bounds =  [1e-4 1e7];
